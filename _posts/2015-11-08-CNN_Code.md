            ...
            if strcmp(net.layers{l}.type, 'c')
                for j = 1 : numel(net.layers{l}.a)
                    for i = 1 : numel(net.layers{l - 1}.a)
                            ...
                            // layers{l}.dk{i}{j}与layers{l}.k{i}{j}的size相同
                    end
                    net.layers{l}.db{j} = sum(net.layers{l}.d{j}(:)) / size(net.layers{l}.d{j}, 3) 
                    //<1 x 1>，就表示第 l 层上第 j 个feature map对应的b值
                end
            end
        end
            ...
            if strcmp(net.layers{l}.type, 'c')
                for j = 1:numel(net.layers{l}.a)
                    for ii = 1:numel(net.layers{l-1}.a)
                            ...
                            // layers{2}.k{ii}{j}<5 x 5>，layers{2}.k{ii}{j}<5 x 5>
                    end
...
### cnntest函数
    ...
    function [er, bad] = cnntest(net, x, y)
        ...
        net = cnnff(net, x);    //跑一次
        [~, h] = max(net.o);
        // ～表示最大值，h表示最大值位置，即预测类别<1 x 50>，50个样本，每个样本的预测类别
        [~, a] = max(y);
        // a表示最大值位置<1 x 50>
        bad = find(h ~= a);
        // 计算预测错误的样本个数
